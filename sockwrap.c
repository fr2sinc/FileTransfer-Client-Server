...
/*------------------------------------------------------------------------*/
//ho modificato la funzione controllando
//il carattere \r e poi \n e anche facendo il controllo con la select
ssize_t my_readline_unbuffered(int fd, void *lptr, size_t my_maxlen)
{
	int n, my_rc, flag_r = 0, flag_n = 0, pos = 0;
	char c, *ptr;
	ptr = lptr;
	...
	{	
		...
		int s;
		if ((s = select(FD_SETSIZE, &cset, NULL, NULL, &tv)) == -1)
		{
			return -1; //se fallisce la select restituisce -1
		}
		//printf("n_value: %d\n", n);
		if (s > 0)
		{
			//---------------------------------------------------------------------//
			if ((my_rc = recv(fd, &c, 1, 0)) == 1)
			{
				*ptr++ = c;
				//-----------------------------------------------------//
				//Controlla, \r seguito da \n
				if (c == '\r')
				{
					flag_r = 1;
					pos = n;
				}
				else if (flag_r == 1 && (n - pos) == 1 && c == '\n')
				{
					flag_n = 1;
					break; // newline salvata
				}
				else if (flag_r == 1 && (n - pos) == 1 && c != '\n')
				{
					flag_r = 0;
			...
			{
				if (n == 1)
					return 0; // nessun dato letto, restituisce zero
				else
					break; // alcuni dati sono stati letti 
			}
			else
			{				
				return -1; //error, errno settata dalla read()
			}
...
//Ho modificato la readn facendo il controllo con la select
