...
int main(int argc, char * argv[]) {
  ...
  while (1) { //read to handle connection (accept)
    ...
    if ((childpid = fork()) < 0) 
      ...
      //processo padre
      close(connfd);
      signal(SIGCHLD, catchaChild);
    } else { //processo figlio      
      close(sockfd);

      //printf("Sono il figlio: %d\n", getpid());
      //sleep(11);//per testare che i processi vanno in parallelo
      while (1) { //read until there are gets from client in a specified connection
        ...
        //printf("result: %d\n", read_result);
        if (read_result == 0) {
          break; //il server chiude la connessione perchè il client ha stabilito la connessione, ma non invia nulla, quindi la readline restituisce zero
          ...
          if (strcmp(initial_chars, "GET ") == 0) {
            ...
            if (filefd != NULL) {
              ...
              if (sendn(connfd, (void * ) & last_modified, 4, MSG_NOSIGNAL) == -1) break;
      ...
      printf("Close(connfd)\n");
      close(connfd);
      exit(0); //si chiude il processo figlio
